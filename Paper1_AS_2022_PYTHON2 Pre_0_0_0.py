...
# developed in a Python 2 environment
...
def LoadSolution(PuzzleName, Solution):
  OK = True
  try:
    FileIn = open(PuzzleName + "S.txt", 'r')
...
def LoadPuzzle(PuzzleGrid, Puzzle, Answer, Solution):
  ...
  PuzzleName = raw_input("Enter puzzle name to load: ")
...
def LoadPartSolvedPuzzle(PuzzleGrid, Puzzle, Answer, Solution):
  ...
  try:
    PuzzleName = Answer[0]
    FileIn = open(PuzzleName + "P.txt", 'r')
    CellInfo = FileIn.readline()
    CellInfo = CellInfo[:-1]
...
def DisplayGrid(PuzzleGrid):
  ...
  for Row in range(1, GRID_SIZE + 1):
    sys.stdout.write('{0:>1}'.format(Row))
    sys.stdout.write('|')    
    for Column in range(1, GRID_SIZE + 1):
      if Column % 3 == 0:
        sys.stdout.write(SPACE + PuzzleGrid[Row][Column] + SPACE)
        sys.stdout.write('|')
      else:
        sys.stdout.write(SPACE + PuzzleGrid[Row][Column] + SPACE)
        sys.stdout.write('.')
...
def KeepPuzzle(PuzzleGrid, Answer):
  if PuzzleGrid[0][0] != 'X':
    ...
    if int(Answer[2]) > 0:
      PuzzleName = Answer[0]
      FileOut = open(PuzzleName + "P.txt", 'w')
  ...
  for Row in range(1, GRID_SIZE + 1):
    for Column in range(1, GRID_SIZE + 1):
      ...
      if not (Entry == Solution[Row][Column] or Entry == SPACE):
        Correct = False
        ErrorCount += 1
        print "You have made an error in row ", Row, " column ", Column
  if not Correct:
    print "You have made ", ErrorCount, " error(s)"
  elif Incomplete:
...
def DisplayResults(Answer): 
  if int(Answer[2]) > 0:
    print "Your score is ", Answer[1]
    print "Your solution for ", Answer[0], " was: "
    for Line in range(3, int(Answer[2]) + 3):
      print Answer[Line]
  else:
      ...
      if PuzzleGrid[0][0] != 'X':
        ...
        if int(Answer[2]) > 0:
          ...
          if Solved:
            ...
            print "Your score so far is ", Answer[1]
        else:
